...
def main() -> None:
    ...
    to_add_or_not = input('Do you want to add a custom log entry? (yes/no): ').strip().lower()
    if to_add_or_not == 'yes':
        while True:
            custom_entry = input('Enter the log entry you want to add (or type "exit" to stop): ').strip()
