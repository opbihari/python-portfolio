"""
AI SAGE API SERVER
Python Flask backend for the Python Ink website.
Handles AI-powered code explanations, snippet generation, and interactive
learning via configurable provider back-ends.

Supported providers: gemini | openai | anthropic | local (mock)
...
Run:
    python AI_Code_Assistant_API.py
"""
...
class GeminiService(AIService):
        ...
        payload = {
            ...
            "systemInstruction": {
                "parts": [
                    {
                        "text": (
                            "You are an expert Python AI Sage. Provide concise, "
                            "accurate, and helpful code explanations and snippets. "
                            "Use proper formatting and clear examples."
                        )
                    }
                ]
            },
    ...
    def generate_snippet(self, prompt: str) -> Dict[str, Any]:
        ...
        full_prompt = (
            f'Write a Python snippet for the following: "{prompt}". '
            "Return JSON with title, desc, and code fields."
        )
    ...
    def explain_code(self, code: str, output: Optional[str] = None) -> str:
        ...
        prompt += (
            ...
            "If there's an error, explain why and how to fix it."
        )
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are an expert Python AI Sage. Generate concise, "
                    "well-documented Python code snippets. Respond in JSON format "
                    "with 'title', 'desc', and 'code' fields."
                ),
            },
            {
                "role": "user",
...
class MockService(AIService):
    ...
    def explain_code(self, code: str, output: Optional[str] = None) -> str:
        ...
        explanation = (
            f"This Python code snippet contains {lines} lines. "
            "It demonstrates basic Python concepts and good coding practices."
        )
...
def refactor_code():
    ...
    try:
        ...
        if not code:
            return (
                jsonify({"success": False, "error": "Code cannot be empty"}),
                400,
            )
        ...
        prompt = (
            "Review this Python code and suggest improvements for readability, "
            f"efficiency, and PEP 8 compliance:\n\n```python\n{code}\n```\n\n"
